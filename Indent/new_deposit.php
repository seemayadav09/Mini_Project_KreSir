...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Prepare inserting the deposit record
    $stmt = $conn->prepare("INSERT INTO deposit(date, accountHolder, accountNumber, ifsc, amount) VALUES(?,?,?,?,?)");
    if (!$stmt) {
        die('Connection Failed : ' . $conn->error);
    }

    // Bind the parameters
    $stmt->bind_param("ssisi", $date, $accountHolder, $accountNumber, $ifsc, $amount);
    
    if ($stmt->execute()) {
        echo "<script>alert('Amount Deposited successfully');</script>";
    } else {
        echo "<script>alert('Error depositing amount: ".$stmt->error."');</script>";
    }
