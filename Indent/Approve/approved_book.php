                  ...
                  if ($total != 0) {
                    while ($result = mysqli_fetch_assoc($data)) {
                      echo "
                        ...
                        <td>" . $result['field'] . "</td>
                        <td>
                            <a href='add_suggested_book.php?
                                book_name=" . urlencode($result["book_name"]) . "&
                                author=" . urlencode($result["author"]) . "&
                                edition=" . urlencode($result["edition"]) . "&
                                isbnnum=" . urlencode($result["isbn"]) . "&
                                publisher=" . urlencode($result["publisher"]) . "&
                                year=" . urlencode($result["year"]) . "&
                                cost=" . urlencode($result["cost"]) . "&
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $("#suggestButton").on("click", function() {
        ...
        $.ajax({
          type: "POST",
          url: "process_approved_books.php", // Updated the URL here
          data: {
            approved_books: JSON.stringify(selectedBooks) // Convert to JSON string
          },
